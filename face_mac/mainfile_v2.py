######################################################
'''
    this is version v2 change some checking statement
    workflow
    
    FROM version 1
    run ---> detecing + scanning --> recog --> check if mac_indx == face_indx
    --> timestamp --> done
    
    TO version 2 
    run -->detecting + scanning  --> check if mac_indx == face_indx
    --> recog --> timestamp--> done
